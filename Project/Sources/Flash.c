...
static uint8_t MemoryMap[FLASH_SIZE] = { CLEAR }; //set it to be clear.
		...
		case 2:
			...
			for (i=0; i<FLASH_SIZE;i++)
			{
				if (MemoryMap[i] == 0)
				{
					...
					if (!memcmp(clearBlock, (void *)&MemoryMap[i], 2))
					{
						//all 0,return true
						memset(&MemoryMap[i], 1, 2);//set this space as taken
						*variable = (void *)FLASH_DATA_START + i;
						return true;
					}
				}
		...
		case 4:
			...
			for (i=0; i<FLASH_SIZE;i++)
			{
				if (MemoryMap[i] == 0)
				{
					//found a byte that has not been allocated yet.
					//compare clear block with this block of memory found
					if (!memcmp(clearBlock, (void *)&MemoryMap[i], 4))
					{
						//all 0,return true
						memset(&MemoryMap[i], 1, 4);//set this space as taken
						*variable = (void *)FLASH_DATA_START + i;
						return true;
					}
				}
